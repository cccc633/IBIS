...
inference <- function(n, data, temp.futile, temp.eff, phi){
  ## This function is used to do inference for one-stage IBIS ##
  ## n, sample size; data, trial data;
  ## temp.futile indicates which subgroups are in the low-efficacy subset
  ## temp.eff indicates which subgroups are in the high-efficacy subset
  ...
  if (length(temp.futile)==1){
    ...
    result.eff <- (apply(output, 2, function(x){mean(x>0)/mean(x<=0)}) > phi)[1:(ncol(data)-1)]
    result.futile <- (apply(output, 2, function(x){mean(x>0)/mean(x<=0)}) > phi)[ncol(data)]
  } 
  if (length(temp.eff)==1){
    ...
    output <- as.data.frame(output[[1]])
    ## When x is the posterior samplings for theta, mean(x>0)/mean(x<=0) denotes Pr(theta>0)/Pr(theta<0)
    result.eff <- (apply(output, 2, function(x){mean(x>0)/mean(x<=0)}) > phi)[1]
    result.futile <- (apply(output, 2, function(x){mean(x>0)/mean(x<=0)}) > phi)[2:ncol(data)]
  }
  if (length(temp.futile)>1 & length(temp.eff)>1){
    jags.data <- list("meanf"=colMeans(data[,temp.futile]), "Nf"=length(temp.futile), 
                      "meane"=colMeans(data[,temp.eff]), "Ne"=length(temp.eff), 
    ...
    output <- as.data.frame(output[[1]])
    ## When x is the posterior samplings for theta, mean(x>0)/mean(x<=0) denotes Pr(theta>0)/Pr(theta<0)
    result.eff <- (apply(output, 2, function(x){mean(x>0)/mean(x<=0)}) > phi)[1:length(temp.eff)]
    result.futile <- (apply(output, 2, function(x){mean(x>0)/mean(x<=0)}) > phi)[(length(temp.eff)+1):ncol(data)]
  }
