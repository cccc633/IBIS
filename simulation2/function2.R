Independent <- function(trial, K, J, n, mu, sigma, phi){
  ...
  FWER <- as.numeric(sum(result[which(mu <= 0)]) > 0)
  ConPower <- 1 - as.numeric(sum(result[which(mu >= 1)]) < sum(mu >= 1))
...
IBIS <- function(trial, K, J, n, mu, sigma, G.futile, phi){
  ...
  ## inference
  result <- c()
  result0 <- inference(n, data, cluster.futile, cluster.eff, phi)
...
Freq <- function(trial, K, J, n, mu, sigma, G.futile, phi){
  ...
  t.max <- which(t.statistic == max(t.statistic))[1]
